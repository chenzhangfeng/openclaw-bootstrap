...
echo ===================================================
echo.
echo   [说明] 面向小白的正式发行包，优先建议重新下载新版压缩包覆盖。
echo   [说明] 本工具更适合仍保留 Git 仓库结构的兼容场景。
...
if not exist "%BASE_DIR%\.git" (
    echo [提示] 当前目录不是 Git 仓库结构，无法使用兼容更新脚本。
    echo [建议] 请返回下载页重新获取最新版本压缩包，并覆盖旧目录。
    pause
    exit /b 1
)
...
echo 若这是正式发行包，后续仍建议以重新下载新版压缩包为主。
